...
def get_args_parser():
    ...
    parser.add_argument('--vit_type', default='', type=str, help="""vit type""")
...
def train_svt(args):
    ...
    if args.arch == "timesformer":
        if args.vit_type == "vit_base":
            student = get_vit_base_patch16_224(cfg=config, no_head=True)
            teacher = get_vit_base_patch16_224(cfg=config, no_head=True)
            embed_dim = student.embed_dim
        elif args.vit_type == "vit_small":
            student = get_vit_small_patch16_224(cfg=config, no_head=True)
            teacher = get_vit_small_patch16_224(cfg=config, no_head=True)
            embed_dim = student.embed_dim
        elif args.vit_type == "vit_tiny":
            student = get_vit_tiny_patch16_224(cfg=config, no_head=True)
            teacher = get_vit_tiny_patch16_224(cfg=config, no_head=True)
            embed_dim = student.embed_dim
