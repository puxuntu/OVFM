...
from datasets import UCF101, HMDB51, Kinetics, PhaseDatasetImages
...
def eval_finetune(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            test_stats, f1, accuracy, precision, recall, jaccard = validate_network(val_loader, model_full, args.n_last_blocks, args.avgpool_patchtokens, epoch)
...
def validate_network(val_loader, model_full, n, avgpool, epoch):
    ...
    img_paths = []
    labels = []
    data_file = "../OVFM/data/downstream/phase_recognition/Cataract 101/" + "val_4_phases_images.txt"
    ...
    for (inp, target, sample_idx, meta) in metric_logger.log_every(val_loader, 20, header):
        ...
        probabilities = F.softmax(output, dim=1)  # Predictions for all classes
        _, predicted_label = torch.max(output, dim=1)  # Predicted class labels
        target_label = target.cpu().numpy()  # Actual target labels for the batch
        # Iterate over the batch (since batch size = 8)
        for i in range(inp.shape[0]):
            # Get the actual index for this sample in the batch
            actual_idx = sample_idx[i].item() if isinstance(sample_idx, torch.Tensor) else sample_idx + i
            if actual_idx >= len(img_paths):
                continue
            
            # Extract video and frame info from the path
            path_parts = img_paths[actual_idx].split('/')
            video_part = [part for part in path_parts if "video" in part][0]
            video_id, frame_id = video_part.replace("video", "").replace(".jpg", "").split('_')
            # Prepare the data to save for each sample in the batch
            row = {
                'video_id': video_id,
                'frame_id': frame_id,
                'probabilities': probabilities[i].cpu().numpy(),
                'predicted_label': predicted_label[i].item(),
    ...
    df.to_excel(f"predictions_{epoch + 1}.xlsx", index=False)

    f1 = f1_score(all_target, all_output, average='weighted')
    # Compute accuracy
    accuracy = accuracy_score(all_target, all_output)
    # Compute precision
    precision = precision_score(all_target, all_output, average='weighted')
    # Compute recall
    recall = recall_score(all_target, all_output, average='weighted')
    # Compute Jaccard similarity score
    jaccard = jaccard_score(all_target, all_output, average='weighted')

    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, f1, accuracy, precision, recall, jaccard
