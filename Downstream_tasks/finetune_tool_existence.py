...
from datasets import UCF101, HMDB51, Kinetics, PhaseDatasetImages, ToolExistenceImages
...
def eval_finetune(args):
    ...
    if args.dataset == "toolExistenceImages":
        dataset_train = ToolExistenceImages(cfg=config, mode="train")
        dataset_val = ToolExistenceImages(cfg=config, mode="val")
    ...
    optimizer = torch.optim.SGD(
        [{'params': model_full.parameters(), 'lr': args.lr}],
        momentum=0.9,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            auc = validate_network(val_loader, model_full, args.n_last_blocks, args.avgpool_patchtokens, epoch)
            print(f"auc at epoch {epoch} of the network on the {len(dataset_val)} test images: {auc * 100:.1f}%")
            ...
            print(f'Max auc so far: {best_auc * 100:.1f}%')

def compute_class_weights(class_counts):
    total_count = sum(class_counts)
    class_weights = [total_count / count for count in class_counts]
    return torch.tensor(class_weights, dtype=torch.float32).cuda()
class_counts = [2493, 3039, 6004, 22078]
...
def train(args, model_full, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, target, sample_idx, image_path, meta) in metric_logger.log_every(loader, 20, header):
        # move to gpu
...
def validate_network(val_loader, model_full, n, avgpool, epoch):
    output_file="tool_predictions_"+str(epoch)+".xlsx"
    model_full.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    ...
    for (inp, target, sample_idx, image_path, meta) in metric_logger.log_every(val_loader, 20, header):
        # move to gpu
        inp = inp.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        ...
        # forward
        with torch.no_grad():
            # if torch.any(target == 0.5):  # Skip if labels contain 0.5
            #     continue
            target[target == 0.5] = 1
            ...
            all_image_paths.extend(image_path)  # Add image paths to list
    ...
    all_probs = 1 / (1 + np.exp(-all_output))
    
    # Compute micro-AUC: flattens the targets and predictions to treat as one big binary task
    try:
        # Reshape to (samples * num_tools)
        y_true_flat = all_target.flatten()
        y_pred_flat = all_probs.flatten()
        
        # Filter out any potential NaN values in targets or predictions
        ...
        # Need both positive and negative examples for AUC calculation
        if len(np.unique(y_true_flat)) > 1:
            micro_auc = roc_auc_score(y_true_flat, y_pred_flat)
            print(f"Micro-AUC: {micro_auc:.4f}")
        else:
            print("Warning: Cannot compute AUC - all targets are the same class")
            micro_auc = None
    ...
    df = pd.DataFrame({'image_path': [os.path.basename(path) for path in all_image_paths]})
    
    # Add columns for each tool (target and prediction)
    num_tools = all_target.shape[1]  # Should be 21
    
    # Also compute per-class AUCs for the Excel file
    class_aucs = []
    for tool_idx in range(num_tools):
        # Add target column
        df[f'tool_{tool_idx+1}_target'] = all_target[:, tool_idx]
        # Add prediction probability column
        df[f'tool_{tool_idx+1}_prob'] = all_probs[:, tool_idx]
        
        # Try to compute per-class AUC
        try:
            if len(np.unique(all_target[:, tool_idx])) > 1:
                class_auc = roc_auc_score(all_target[:, tool_idx], all_probs[:, tool_idx])
                class_aucs.append(class_auc)
                print(f"Tool {tool_idx+1} AUC: {class_auc:.4f}")
            else:
                class_aucs.append(None)
                print(f"Tool {tool_idx+1}: Cannot compute AUC - all targets are the same class")
        except Exception as e:
            class_aucs.append(None)
            print(f"Error computing AUC for tool {tool_idx+1}: {e}")
    
    # Add a summary page with AUC values
    summary_df = pd.DataFrame({
        'Tool': [f'Tool {i+1}' for i in range(num_tools)] + ['Micro Average'],
        'AUC': class_aucs + [micro_auc]
    })
    
    # Save to Excel with two sheets - detailed results and summary
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_tools', default=1000, type=int, help='Number of labels for linear classifier')
