...
def connectivity_loss(pred, smooth=1e-6):
    ...
    for i in range(batch_size):
        binary_pred = (pred[i, 0] > 0.5).float().cpu().numpy()
        labeled_pred, num = label(binary_pred, connectivity=2, return_num=True)
        loss += (num - 1)
...
def eval(model, val_loader, device, classes, epoch):
    all_dice = []
    all_hd95 = []
    model.eval()
    ...
    save_path = "Seg_Dice_data_" + str(epoch)
    
    with open(save_path, 'w') as f:
        for value in all_dice:
            f.write(f"{value}\n")
...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # loss_conn = connectivity_loss(outputs)
            loss = 0.5 * loss_ce + 0.5 * loss_dice
            # loss = 0.45 * loss_ce + 0.45 * loss_dice + 0.1 * loss_conn
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        test_dice, test_hd95 = eval(model, testloader, 'cuda', classes=2, epoch= epoch_num) 
