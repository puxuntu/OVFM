...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, T, H, W = x.shape # [B, C, T, 224, 224]
        x = rearrange(x, 'b c t h w -> (b t) c h w') # [BT, C, 224, 224]
        x = self.proj(x) # [BT, 768, 14, 14]
        W = x.size(-1) # W = 14
        x = x.flatten(2).transpose(1, 2) # [BT, 768, 196] -> [BT, 196, 768 ]
        return x, T, W
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, get_all=False, get_attn=False):
        ...
        B = x.shape[0] # x: [B, C, T, 224, 224]
        x, T, W = self.patch_embed(x) # x: [BT, 196, 768 ] W: 14
        
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1) # cls_tokens: [BT, 1, 768]
        x = torch.cat((cls_tokens, x), dim=1) # x: [BT,197,768]
        # print("x_after_cls_embed:", x.shape)
        # resizing the positional embeddings in case they don't match the input at inference
        if x.size(1) != self.pos_embed.size(1): # pos_embed: [1, 197, 768]
            ...
            x = x + self.pos_embed # x: [BT,197,768]
        x = self.pos_drop(x)
        # print("x_after_pos_embed:", x.shape)
        ...
        if self.attention_type != 'space_only':
            ...
            if T != self.time_embed.size(1):
                ...
                x = x + self.time_embed
            # print("x_after_time_embed:",x.shape) 
            x = self.time_drop(x)
            x = rearrange(x, '(b n) t m -> b (n t) m', b=B, t=T)
            x = torch.cat((cls_tokens, x), dim=1)
        
        # print("x_after_times_embed:", x.shape) # [B, 1569, 768]
        ...
        if get_all:
            return x
        # print("x_return:",x[:, 0].shape)
        return x[:, 0]

    def forward(self, x, use_head=False):
        # print("input_x_shape:",x.shape) #[B,3,T,224,224]
        x = self.forward_features(x)
        # print("output_x_shape:",x.shape) #[B,embed_dim]
...
def get_vit_base_patch16_224(cfg, no_head=False, **kwargs): # with parameters of 122M
    patch_size = 16
...
def get_vit_small_patch16_224(cfg, no_head=False, **kwargs): # with parameters of 
    patch_size = 16
    vit = VisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=cfg.MODEL.NUM_CLASSES,
                            patch_size=patch_size, embed_dim=512, depth=8, num_heads=8, mlp_ratio=4,
                            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
                            attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES,
                            attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, **kwargs)
    vit.attention_type = cfg.TIMESFORMER.ATTENTION_TYPE
...
def get_vit_tiny_patch16_224(cfg, no_head=False, **kwargs): # with parameters of 8M
    patch_size = 16
    vit = VisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=cfg.MODEL.NUM_CLASSES,
...
def get_vit_small_patch16_224_inference(no_head=False): # with parameters of 8M
    patch_size = 16
    vit = VisionTransformer(img_size=224, num_classes=2,
                            patch_size=patch_size, embed_dim=512, depth=8, num_heads=8, mlp_ratio=4,
                            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
                            attn_drop_rate=0., drop_path_rate=0.1, num_frames=8,
...
def get_vit_tiny_patch16_224_inference(no_head=False): # with parameters of 8M
    patch_size = 16
    vit = VisionTransformer(img_size=224, num_classes=2,
                            patch_size=patch_size, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4,
                            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
...
def get_vit_base_patch16_224_inference(no_head=False): 
    patch_size = 16
    vit = VisionTransformer(img_size=224, num_classes=2,
                            patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
                            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
...
# def get_vit_tiny_patch16_224(cfg, no_head=False, **kwargs):
#     patch_size = 16
#     vit = VisionTransformer(img_size=224, num_classes=1000,
#                             patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
#                             qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
#                             attn_drop_rate=0., drop_path_rate=0.1, num_frames=8,
...
#     vit_model = get_vit_tiny_patch16_224(cfg, no_head=False)
...
def build_vit_tiny_patch16_224():
    from utils.parser import parse_args, load_config

    opt = parse_args()
    opt.cfg_file = "models/configs/Kinetics/TimeSformer_divST_8x32_224.yaml"
    config = load_config(opt)
    model = get_vit_tiny_patch16_224(cfg=config)
