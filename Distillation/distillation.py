...
from models import get_vit_base_patch16_224, get_vit_small_patch16_224, get_vit_tiny_patch16_224, get_aux_token_vit, SwinTransformer3D, S3D
...
def get_args_parser():
    ...
    # teacher model path
    parser.add_argument('--teacher_model_path', default='/path/to/teacher/model/path', type=str,
    ...
    parser.add_argument('--student_vit', default='vit_small', type=str,
                        help="""Name of architecture to train. For quick experiments with ViTs,
        we recommend using vit_tiny or vit_small.""")
...
def train_svt(args):
    ...
    if args.arch == "timesformer":
        if args.student_vit == "vit_small":
            student_backbone = get_vit_small_patch16_224(cfg=config, no_head=True)
            print("**************vit_small****************")
        elif args.student_vit == "vit_tiny":
            student_backbone = get_vit_tiny_patch16_224(cfg=config, no_head=True)
            print("**************vit_tiny****************")
        teacher_backbone = get_vit_base_patch16_224(cfg=config, no_head=True)
        embed_dim_student = student_backbone.embed_dim
        embed_dim_teacher = teacher_backbone.embed_dim
    ...
    teacher = utils.MultiCropWrapper(
        teacher_backbone,
        DINOHead(embed_dim_teacher, 
        args.out_dim, 
        args.use_bn_in_head),
        vary_fr=config.DATA.RAND_FR
    )

    pretrained_weights_teacher = torch.load(args.teacher_model_path)['teacher']
    msg = teacher.load_state_dict(pretrained_weights_teacher, strict=False)
    ...
    teacher_without_ddp.load_state_dict(pretrained_weights_teacher)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_dict = {
            ...
            'student_backbone': student_backbone.state_dict(),  # Add this line
